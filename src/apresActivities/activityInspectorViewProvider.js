...
export default class ActivityInspectorViewProvider {
    ...
    /**
    @param {TODO} openmct
    @param {ActionAttribute[]} actionAttributes
    ...
    */
    canView(selection) {
        ...
        return parent && parent.type === 'apres.timeline.type' && object && object.type === 'apres.activity.type'
    }
                ...
                component = new Vue({
                    ...
                    data: () => {
                        return {
                            ...
                            selectedAction: this.#getSelectedAction(selection),
    ...
    */
    #getSelectedAction(selection) {
        ...
        const selectionActionId = this.#getActionObject(selection).id

        // The last three properties are possibly undefined as per the Schemas.
        // This is not ideal, especially since the assumption is that these
        // fields exist.
        const actions = this.#projectJSON.planningProject?.activityPlan?.actions
    ...
    */
    #getUniqueAttributes(selection) {
        ...
        const actionType = this.#projectJSON.interfaceModel.actionTypes.find(
            actionType => actionType.name === actionTypeName,
        )

        if (!actionType) throw new Error('Action type not found. This should not happen.')

        /** @type {ActionAttribute[] | undefined} */
        const uniqueAttributes = actionType.parameters?.map(param => ({
            name: param.name,
            units: param.units,

            // Is this correct for the initial values to show in the UI?
            default:
    ...
    */
    #getDurationFormula(selection) {
        ...
        const actionType = this.#projectJSON.interfaceModel.actionTypes.find(
            actionType => actionType.name === selectedAction.actionType,
        )

        if (!actionType) throw new Error('Not possible.')
